...
def chat_with_assistant():
    ...
    try:
        ...
        if recent_shifts:
            ...
            for i in range(1, len(recent_shifts)):
                prev_date = recent_shifts[i-1].ShiftDate
                curr_date = recent_shifts[i].ShiftDate
        ...
        if result.get('requires_escalation'):
            try:
                alert = BurnoutAlert(
                    ...
                    Severity='critical',
                    AlertMessage=f"Crisis detected in chat conversation",
